...
static const sc202cs_reginfo_t init_reglist_640x480_30fps[] = {
    ...
    {SC202CS_REG_SLEEP_MODE, 0x00},     // Normal operation
    {0x36e9, 0x80},
    {0x36ea, 0x06},
    {0x36eb, 0x0a},
    {0x36ec, 0x01},
    ...
    {0x3200, 0x00},   // X_START high byte
    {0x3201, 0x00},   // Y_START high byte (set to 0 for simplicity)
    {0x3202, 0x02},   // X_END   high byte (0x02)
    {0x3203, 0x7F},   // X_END   low  byte (0x7F)
    {0x3204, 0x01},   // Y_END   high byte (0x01)
    {0x3205, 0xDF},   // Y_END   low  byte (0xDF)
    {0x3206, 0x03},   // X_TOTAL high (kept from 1280×720)
    {0x3207, 0xC7},   // X_TOTAL low  (kept from 1280×720)
    {0x3208, 0x05},   // Y_TOTAL high (kept from 1280×720)
    {0x3209, 0x00},   // Y_TOTAL low  (kept from 1280×720)
    {0x320A, 0x02},   // X_OUTPUT_SIZE high (640 → 0x02)
    {0x320B, 0x80},   // X_OUTPUT_SIZE low  (640 → 0x80)
